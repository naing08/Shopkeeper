...
import {deactivateUserAccountMutation,saveUserAccountMutation,savePasswordMutation} from '../../../common/apollo/UserAccount/index';
class savePasswordDialog extends React.Component{
	...
	updateUserName(){
		let {UserAccountEdit,saveUserAccount} = this.props;
    	...
    	.catch(()=>{this.setState({saving:false,loadingMessage:'',editingUserName:false})});
    }
    savePassword(){
    	let {UserAccountEdit,editUserName} = this.props;
    	...
    	.catch(()=>{this.setState({saving:false,loadingMessage:'',editUserName:false})});
    }
    deactivateAccount(){
    	this.setState({deactivating:true});
    	this.props.deactivateUserAccount(this.props.UserAccountEdit.id)
    	.then(()=>{this.setState({deactivating:false});})
    	.catch(()=>{this.setState({deactivating:false});});
    }
